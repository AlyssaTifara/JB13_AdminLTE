...
// eksekusi query untuk menyimpan ke database
sqlsrv_query($this->db, "insert into {$this->table} (username, nama, level, password) values(?,?,?,?)", array($data['username'], $data['nama'], $data['level'], 
password_hash($data['password'], PASSWORD_DEFAULT)));
}
}
public function getData(){
    ...
    $data = [];
    while($row = sqlsrv_fetch_array($query, SQLSRV_FETCH_ASSOC)){$data[] = $row;}
    return $data;
    }
    }
    ...
    // query untuk update data
    sqlsrv_query($this->db, "update {$this->table} set username = ?, nama = ?, level = ?, password = ? where user_id = ?", [$data['username'], $data['nama'], $data['level'], 
    password_hash($data['password'], PASSWORD_DEFAULT), $id]);
    }
    }
